...
public class ReservasPasajeroMDB {
	...
	private final static String GLOBAL_TOPIC_NAME = "java:global/RMQTopicReservasPasajero";
	private final static String LOCAL_TOPIC_NAME = "java:global/RMQReservasPasajeroLocal";
	...
	private List<ReservaPasajero> answer = new ArrayList<ReservaPasajero>();
	
	public ReservasPasajeroMDB(TopicConnectionFactory factory, InitialContext ctx) throws JMSException, NamingException 
	{	
	...
	public ListaReservasPasajero getRemoteReservasPasajero() throws JsonGenerationException, JsonMappingException, JMSException, IOException, NonReplyException, InterruptedException, NoSuchAlgorithmException
	{
		answer.clear();
...
//		id = new String(md.digest(id.getBytes()));
		...
		ListaReservasPasajero res = new ListaReservasPasajero(answer);
        return res;
	}
	...
	private void sendMessage(String payload, String status, Topic dest, String id) throws JMSException, JsonGenerationException, JsonMappingException, IOException
	{
		...
		ExchangeMsg msg = new ExchangeMsg("reservaspasajero.general.app1", APP, payload, status, id);
		...
		try 
		{
			...
			if(!ex.getSender().equals(APP))
			{
				if(ex.getStatus().equals(REQUEST))
				{
					...
					ListaReservasPasajero reservas = dtm.getLocalReservasPasajero();
					String payload = mapper.writeValueAsString(reservas);
					Topic t = new RMQDestination("", "reservaspasajero.test", ex.getRoutingKey(), "", false);
					sendMessage(payload, REQUEST_ANSWER, t, id);
				}
				else if(ex.getStatus().equals(REQUEST_ANSWER))
				{
					ListaReservasPasajero v = mapper.readValue(ex.getPayload(), ListaReservasPasajero.class);
					answer.addAll(v.getReservasPasajero());
				}
			}
