...
public class VuelAndesAerolineasServices {
	...
	@Path("/nombre/{nombre}")
	@Produces({ MediaType.APPLICATION_JSON })
	public Response getAerolineasNombre(@javax.ws.rs.PathParam("nombre") String nombre) {
		VuelAndesMaster tm = new VuelAndesMaster(getPath());
		ListaAerolineas aerolineas;
		try {
			if (nombre == null || nombre.length() == 0)
				throw new Exception("Nombre del admin no valido");
			aerolineas = tm.buscarAerolineaPorNombre(nombre);
		} catch (Exception e) {
			return Response.status(500).entity(doErrorMessage(e)).build();
		}
		return Response.status(200).entity(aerolineas).build();
	...
	@Path("/iata/{iata}")
	@Produces({ MediaType.APPLICATION_JSON })
	public Response getAerolineasIata(@javax.ws.rs.PathParam("iata") String iata) {
		VuelAndesMaster tm = new VuelAndesMaster(getPath());
		ListaAerolineas aerolineas;
		try {
			if (iata == null || iata.length() == 0)
				throw new Exception("Iata de la aerolinea no valido");
			aerolineas = tm.buscarAerolineaPorIata(iata);
		} catch (Exception e) {
			return Response.status(500).entity(doErrorMessage(e)).build();
		}
		return Response.status(200).entity(aerolineas).build();
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response addAerolineas(ListaAerolineas aerolineas) {
		VuelAndesMaster tm = new VuelAndesMaster(getPath());
		try {
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response deleteAdministrador(Aerolinea aerolinea) {
		VuelAndesMaster tm = new VuelAndesMaster(getPath());
		try {
			tm.deleteAerolineas(aerolinea);
		} catch (Exception e) {
			return Response.status(500).entity(doErrorMessage(e)).build();
		}
		return Response.status(200).entity(aerolinea).build();
