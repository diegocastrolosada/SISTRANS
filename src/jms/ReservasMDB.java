...
public class ReservasMDB implements ExceptionListener, MessageListener {
	...
	private final static String GLOBAL_TOPIC_NAME = "java:global/RMQTopicAllReservas";
	private final static String LOCAL_TOPIC_NAME = "java:global/RMQAllReservasLocal";
	...
	private List<Reserva> answer = new ArrayList<Reserva>();
	
	public ReservasMDB(TopicConnectionFactory factory, InitialContext ctx) throws JMSException, NamingException 
	{	
...
//		id = new String(md.digest(id.getBytes()));
		...
		ListaReservas res = new ListaReservas(answer);
        return res;
	}
		...
		try 
		{
			...
			if(!ex.getSender().equals(APP))
			{
				if(ex.getStatus().equals(REQUEST))
				{
					...
					ListaReservas reservas = dtm.getLocalReservas();
				...
				{
					ListaReservas v = mapper.readValue(ex.getPayload(), ListaReservas.class);
