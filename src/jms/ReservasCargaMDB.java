...
public class ReservasCargaMDB implements MessageListener, ExceptionListener {
	public final static int TIME_OUT = 5;
	private final static String APP = "app1";
	
	private final static String GLOBAL_TOPIC_NAME = "java:global/RMQTopicReservasCarga";
	private final static String LOCAL_TOPIC_NAME = "java:global/RMQReservasCargaLocal";
	...
	private List<ReservaCarga> answer = new ArrayList<ReservaCarga>();
	
	public ReservasCargaMDB(TopicConnectionFactory factory, InitialContext ctx) throws JMSException, NamingException 
	{	
	...
	public ListaReservasCarga getRemoteReservasCarga() throws JsonGenerationException, JsonMappingException, JMSException, IOException, NonReplyException, InterruptedException, NoSuchAlgorithmException
	{
		answer.clear();
...
//		id = new String(md.digest(id.getBytes()));
		...
		ListaReservasCarga res = new ListaReservasCarga(answer);
        return res;
	}
		...
		try 
		{
			...
			if(!ex.getSender().equals(APP))
			{
				if(ex.getStatus().equals(REQUEST))
				{
					...
					ListaReservasCarga reservas = dtm.getLocalReservasCarga();
					String payload = mapper.writeValueAsString(reservas);
					Topic t = new RMQDestination("", "reservasCarga.test", ex.getRoutingKey(), "", false);
					sendMessage(payload, REQUEST_ANSWER, t, id);
				}
				else if(ex.getStatus().equals(REQUEST_ANSWER))
				{
					ListaReservasCarga v = mapper.readValue(ex.getPayload(), ListaReservasCarga.class);
					answer.addAll(v.getReservasCarga());
				}
			}
